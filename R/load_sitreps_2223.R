#' Load timeseries of winter situation reports for 2022-23
#'
...
#' @param ambo_url URL for ambulance sitrep timeseries
#'
#' @importFrom magrittr "%>%"
#'
load_sitreps_2223 = function(sitrep_url, ambo_url) {
  ##
  ## load sitrep timeseries
  ##
  # download sitrep files
  tmp_sitrep = tempfile()
  download.file(sitrep_url, tmp_sitrep, mode = "wb")

  tmp_ambo = tempfile()
  download.file(ambo_url, tmp_ambo, mode = "wb")
  ...
  sitrep_ambo       = readxl::read_excel(tmp_ambo, sheet = "Ambulance Arrivals and Delays", skip = 14, .name_repair = my_repair_names)
  ...
  sitrep_dates$Occupancy_adult_void = c('Adult G&A beds unavailable to non-covid admissions “void”',
                                        paste('Adult G&A beds unavailable to non-covid admissions “void”', seq(1:num_cols), sep="__"))
  ...
  sitrep_dates$Occupancy_paeds_void = c('Paeds G&A beds unavailable to non-covid admissions “void”',
                                        paste('Paeds G&A beds unavailable to non-covid admissions “void”', seq(1:num_cols), sep="__"))
  ...
  sitrep_beds_adult = sitrep_beds_adult %>%
    ...
    dplyr::select(Code, Name, Region = `NHS England Region`, dplyr::starts_with("Adult")) %>%   # keep only bed occupancy rates
    janitor::remove_empty("rows") %>%
    dplyr::mutate(dplyr::across(-c(Code, Name, Region), as.integer))
  ...
  sitrep_beds_paed = sitrep_beds_paed %>%
    dplyr::slice(-c(3)) %>%   # skip blank line
    dplyr::select(Code, Name, Region = `NHS England Region`, dplyr::starts_with("Paeds")) %>%   # keep only bed occupancy rates
    janitor::remove_empty("rows") %>%
    dplyr::mutate(dplyr::across(-c(Code, Name, Region), as.integer))
  ...
  sitrep_beds = dplyr::bind_rows(
    sitrep_beds_adult %>%
      tidyr::gather(Occupancy, `Occupancy rate`, -Code, -Name, -Region) %>%
  ...
  sitrep_beds = sitrep_beds %>%
    dplyr::group_by(Code, Name, Region, Occupancy, Date) %>%
  ...
  sitrep_critical = sitrep_critical %>%
    dplyr::slice(-c(3)) %>%   # skip blank line
    dplyr::select(Code, Name, Region = `NHS England Region`, dplyr::starts_with("CC")) %>%   # keep only bed occupancy rates
    janitor::remove_empty("rows") %>%
    dplyr::mutate(dplyr::across(-c(Code, Name, Region), as.integer))
  ...
  sitrep_beds_long_7 = sitrep_beds_long %>%
    dplyr::slice(-c(3)) %>%   # skip blank line
    dplyr::select(Code, Name, Region = `NHS England Region`, dplyr::starts_with("> 7 days"))  # keep only people in for more than three weeks

  # convert to long format
  sitrep_beds_long_7 = sitrep_beds_long_7 %>%
    tidyr::gather(LongStay, `No. beds occupied by long-stay patients (> 7 days)`, -Code, -Name, -Region) %>%
  ...
  sitrep_beds_long_21 = sitrep_beds_long %>%
    dplyr::slice(-c(3)) %>%   # skip blank line
    dplyr::select(Code, Name, Region = `NHS England Region`, dplyr::starts_with("> 21 days"))  # keep only people in for more than three weeks

  # convert to long format
  sitrep_beds_long_21 = sitrep_beds_long_21 %>%
    tidyr::gather(LongStay, `No. beds occupied by long-stay patients (> 21 days)`, -Code, -Name, -Region) %>%
  ...
  sitrep_flu_GA = sitrep_beds_flu %>%
    dplyr::slice(-c(3)) %>%   # skip blank line
    dplyr::select(Code, Name, Region = `NHS England Region`, dplyr::starts_with("G&A"))

  # convert to long format
  sitrep_flu_GA = sitrep_flu_GA %>%
    tidyr::gather(Flu, `No. beds occupied by flu patients (G&A)`, -Code, -Name, -Region) %>%
  ...
  sitrep_flu_CC = sitrep_beds_flu %>%
    dplyr::slice(-c(3)) %>%   # skip blank line
    dplyr::select(Code, Name, Region = `NHS England Region`, dplyr::starts_with("CC"))

  # convert to long format
  sitrep_flu_CC = sitrep_flu_CC %>%
    tidyr::gather(Flu, `No. beds occupied by flu patients (Critical Care)`, -Code, -Name, -Region) %>%
  ...
  sitrep_ambo30 = sitrep_ambo %>%
    dplyr::slice(-c(2:3)) %>%   # skip blank line
    dplyr::select(Code, Name, Region = `NHS England Region`, dplyr::starts_with("Delay 30"))

  # convert to long format
  sitrep_ambo30 = sitrep_ambo30 %>%
    tidyr::gather(Ambo30, Delays, -Code, -Name, -Region) %>%
  ...
  sitrep_ambo60 = sitrep_ambo %>%
    dplyr::slice(-c(2:3)) %>%   # skip blank line
    dplyr::select(Code, Name, Region = `NHS England Region`, dplyr::starts_with("Delay >"))

  # convert to long format
  sitrep_ambo60 = sitrep_ambo60 %>%
    tidyr::gather(Ambo60, Delays, -Code, -Name, -Region) %>%
  ...
  sitrep_diverts = sitrep_diverts %>%
    dplyr::slice(-c(3)) %>%   # skip blank line
    dplyr::select(-V__1) %>%
    dplyr::rename(Region = `NHS England Region`)

  # convert to long format
  sitrep_diverts = sitrep_diverts %>%
    tidyr::gather(Date, `Diverts`, -Code, -Name, -Region) %>%
  ...
  # convert to long format
  sitrep_closures = sitrep_closures %>%
    tidyr::gather(Date, Closures, -Code, -Name, -Region) %>%
    dplyr::mutate(Date = janitor::excel_numeric_to_date(as.integer(Date)))  # convert numbers to dates
  ...
  ##
  # Collate a complete list of Trusts, regions and dates for joining all the other data to
  sitrep_trusts = dplyr::bind_rows(
    sitrep_beds        %>% dplyr::select(Code, Name, Region, Date),
    sitrep_critical    %>% dplyr::select(Code, Name, Region, Date),
    sitrep_closures    %>% dplyr::select(Code, Name, Region, Date),
    sitrep_diverts     %>% dplyr::select(Code, Name, Region, Date),
    sitrep_beds_long_7 %>% dplyr::select(Code, Name, Region, Date),
    sitrep_ambo30      %>% dplyr::select(Code, Name, Region, Date),
    sitrep_flu_GA      %>% dplyr::select(Code, Name, Region, Date)
  ) %>%
    dplyr::mutate(Name = dplyr::if_else(stringr::str_detect(toupper(Name), "ENGLAND"), "England", Name)) %>%
    dplyr::distinct() %>%
    na.omit()
  ...
  # Filter out Type 1 Acute Trusts
  sitrep_trusts = dplyr::filter(sitrep_trusts, Name != "ENGLAND (Type 1 Acute Trusts)")
  sitrep_beds = dplyr::filter(sitrep_beds, Name != "ENGLAND (Type 1 Acute Trusts)")
  sitrep_critical = dplyr::filter(sitrep_critical, Name != "ENGLAND (Type 1 Acute Trusts)")
  sitrep_beds_long_7 = dplyr::filter(sitrep_beds_long_7, Name != "ENGLAND (Type 1 Acute Trusts)")
  sitrep_beds_long_21 = dplyr::filter(sitrep_beds_long_21, Name != "ENGLAND (Type 1 Acute Trusts)")
  sitrep_flu_GA = dplyr::filter(sitrep_flu_GA, Name != "ENGLAND (Type 1 Acute Trusts)")
  sitrep_flu_CC = dplyr::filter(sitrep_flu_CC, Name != "ENGLAND (Type 1 Acute Trusts)")
  sitrep_closures = dplyr::filter(sitrep_closures, Name != "ENGLAND (Type 1 Acute Trusts)")
  sitrep_diverts = dplyr::filter(sitrep_diverts, Name != "ENGLAND (Type 1 Acute Trusts)")

  sitrep = sitrep_trusts %>%
    dplyr::left_join(sitrep_beds         %>% dplyr::select(-Name, -Region), by = c("Code", "Date")) %>%
    ...
    dplyr::left_join(sitrep_critical     %>% dplyr::select(-Name, -Region), by = c("Code", "Date")) %>%
    ...
    dplyr::left_join(sitrep_flu_GA       %>% dplyr::select(Code, Date, `No. beds occupied by flu patients (G&A)`),  by = c("Code", "Date")) %>%
    dplyr::left_join(sitrep_flu_CC       %>% dplyr::select(Code, Date, `No. beds occupied by flu patients (Critical Care)`),  by = c("Code", "Date")) %>%
  ...
  # re-order columns
  sitrep = sitrep %>%
    dplyr::select(Code, Name, Region, Date, dplyr::everything()) %>%
    dplyr::distinct() %>%
